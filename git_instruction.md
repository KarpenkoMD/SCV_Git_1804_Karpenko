...
Все новые изменения принято делать в новой ветке. 

* `git push`
Для выгрузки на ресурс **GitHUb** нам для начала необходимо создать на этом ресурсе репозиторий. 
...
```
Таким образом, первой строчкой мы...  
Второй строчкой...  
Третей строчкой отправляем наш преокт на удаленнй репо
...
* `git pull <URL>` - также забирает данные с удаленного репо, но уже сливает с данныеми на локальном компьютере. По сути, это комбинация двух команд `fethc` и `merge`.
